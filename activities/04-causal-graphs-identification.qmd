...
Slides for today are available [here](https://docs.google.com/presentation/d/1jvQNG1nUVomW8p2Ft__wvHU-SdYvIkHlr0asWpttMIs/edit?usp=sharing).
...
Then for each of the graphs, identify the set of variables that would block noncausal paths between the treatment $A$ and outcome $Y$. ($U$ and $W$ are unmeasured.)

Check your answers to one of the graphs using [DAGitty](http://www.dagitty.net/dags.html). To do this:

- Input the graph on the canvas.
- Click on the node (S) that has been selected/conditioned on, and label it as "adjusted" in the top left menu.
    - In the top right box, check the variable sets under "Minimal sufficient adjustment sets containing S for estimating the total effect of A on Y". These sets of variables are needed to block noncausal paths.
- Alternatively, click on the node (S) that has been selected/conditioned on, and label it as "selected" in the top left menu.
    - The top right box will say "Biasing paths are open", but you can look at the "Testable implications" box on the right and look for the entries that start with "A ⊥ Y". The ⊥ symbol stands for "independent". So "A ⊥ Y | Z" means "A and Y are conditionally independent given Z".
    - Note that this approach only works if there is no arrow between treatment and outcome because any arrow would make A and Y related no matter what, and A and Y wouldn't show up in this conditional independence list.
